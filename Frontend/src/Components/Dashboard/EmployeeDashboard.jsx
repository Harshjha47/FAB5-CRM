...
const EmployeeDashboard = () => {
    // 1. Added 'loading' here
    const { allData, activeTab, setActiveTab, statusFilter, setStatusFilter, user, getDashboardData, loading } = useAuth();
    const [data, setData] = useState(allData || []);
    
    // 2. Added user?.role to the dependency array
    useEffect(() => { 
        if (user?.role === "owner") {
    ...
    // Prevent random fetches if AuthContext is still figuring out who the user is
    useEffect(() => {
        if (!allData && !loading) {
            getDashboardData();
        }
    ...
    // 3. Prevent render until both 'loading' (user) and 'allData' are finished
            ...
            <div className="bg-white rounded-lg shadow overflow-x-auto">
                <table className="w-full text-left border-collapse">
                    ...
                    <tbody className="text-sm border-black">
                            ...
                            <tr key={conn._id} className="border-b hover:bg-gray-50">
                                ...
                                <td className="p-4">
                                    <span className={`px-2 py-1 rounded-full text-xs ${conn?.status === 'Pending' ? 'bg-yellow-100 text-yellow-700' : 'bg-green-100 text-green-700'}`}>
                                        {/* 5. Fixed user.role crash and removed rogue {} here */}
                        ...
                        {activeTab === 'users' && displayData?.map((u) => (
                            <tr key={u._id || u.id} className="border-b hover:bg-gray-50">
                                ...
                                <td className="p-4">
                                    <Link to={`/employees/${u?._id}`} className="border border-[#0000ff2a] text-[#00001f] font-semibold px-4 rounded-md bg-[#e0deff27]">View</Link>
                                </td>
                            </tr>
                        ))}
                    </tbody>
